 ...
 */
public class OrderHistoryAdapterForNewClient extends RecyclerView.Adapter<OrderHistoryAdapterForNewClient.OrderHistoryViewHolder> {
    private final Context context;

    private List<OrderHistory> items, filterList;

    public OrderHistoryAdapterForNewClient(Context context, List<OrderHistory> items) {
        this.items = items;
    ...
    public OrderHistoryViewHolder onCreateViewHolder(ViewGroup parent,
        ...
        View v = LayoutInflater.from(parent.getContext())
                .inflate(R.layout.row_order_list_new, parent, false);
    ...
    public void onBindViewHolder(OrderHistoryViewHolder holder, final int position) {
        ...
        if (!item.getDrug().equalsIgnoreCase("")) {
            if (!item.getDrug_qty().equalsIgnoreCase("")) {
                holder.txt_qty.setText("Qty: " + item.getDrug_qty());
            }
        } else {
            holder.txt_qty.setText(null);
        }
...
//This makes the dialog take up the full width
        ...
        String LastThereeCharacter = item.getDocumentPath().substring(item.getDocumentPath().length() - 3);
        ...
        if (LastThereeCharacter.equalsIgnoreCase("pdf")) {
            ...
            webView.loadUrl("http://docs.google.com/gview?embedded=true&url=" + item.getDocumentPath());
